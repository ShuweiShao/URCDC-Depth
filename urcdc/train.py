...
def main_worker(gpu, ngpus_per_node, args):
    ...
    while epoch < args.num_epochs:
        ...
        for step, sample_batched in enumerate(dataloader.data):
            ...
            loss3 =  0.1*criterion_diff(depth_est, depth_est2.detach(), u2_gt.detach(),mask.float())
            loss4 =  0.1*criterion_diff(depth_est2, depth_est.detach(), u1_gt.detach(),mask.float())
            ...
            if args.do_online_eval and global_step and global_step % args.eval_freq == 0 and not model_just_loaded:
                ...
                if eval_measures is not None and eval_measures2 is not None:
                    ...
                    with open(log_txt, 'a') as txtfile:
                        ...
                        txtfile.write("Transformer\n")
                        line = ''
                        line2 = ''
                        for i in range(9):
                            line +='{:7.4f}, '.format(eval_measures[i])
                            line2 +='{:7.4f}, '.format(eval_measures2[i])
                        txtfile.write(line+'\n')
                        txtfile.write("CNN\n")
                        txtfile.write(line2+'\n')
...
def main():
    ...
    if save_files:
        ...
        command = 'cp urcdc/train.py ' + aux_out_path
        os.system(command)
        command = 'mkdir -p ' + networks_savepath + ' && cp urcdc/networks/*.py ' + networks_savepath
        os.system(command)
        command = 'mkdir -p ' + dataloaders_savepath + ' && cp urcdc/dataloaders/*.py ' + dataloaders_savepath
        os.system(command)
